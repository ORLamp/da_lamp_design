...
//                CvType.CV_8UC1);
        ...
        Mat upperBlueRange = new Mat(hsvImage.rows(), hsvImage.cols(),
                CvType.CV_8UC1);
        Core.inRange(hsvImage, new Scalar(0, 100, 100), new Scalar(20, 255, 255), lowerBlueRange);
...
//        Imgproc.cvtColor(inputFrame, grayMat, colorChannels);
        ...
        Imgproc.HoughCircles(blueImage, circles, CV_HOUGH_GRADIENT, 1, minDist, param1, param2, minRadius, maxRadius);
        ...
        center.x = mOpenCvCameraView.mFrameWidth / 2;
        center.y = mOpenCvCameraView.mFrameHeight / 2;

        sendCommandToLamp(currentPosition, center);

        return inputFrame;
    }

    public void sendCommandToLamp(Position mPosition, Position center) {
        if (connected) {
            if (mMotionControl.onDxDy(mPosition, center).x > 20) {
                serialPort.write("1,80".getBytes());
                Log.d("MoveX", "1,80");
            } else if (mMotionControl.onDxDy(mPosition, center).x < -20) {
                serialPort.write("1,100".getBytes());
                Log.d("MoveX", "1,100");
            }

            if (mMotionControl.onDxDy(mPosition, center).y > 20) {
                serialPort.write("2,80".getBytes());
                Log.d("MoveY", "2,80");
            } else if (mMotionControl.onDxDy(mPosition, center).y < -20) {
                serialPort.write("2,100".getBytes());
                Log.d("MoveY", "2,100");
            }
        } else {
        }
        ...
        if (!usbDevices.isEmpty()) {
            ...
            for (Map.Entry<String, UsbDevice> entry : usbDevices.entrySet()) {
                ...
                if (deviceVID == 0x2341)//Arduino Vendor ID
                {
                    ...
                    startButton.setVisibility(SurfaceView.GONE);
                } else {
                    connection = null;
                    device = null;
                    connected = false;
...
//        tvAppend(textView, "Data Sent : " + string + "\n");
//
//    }
